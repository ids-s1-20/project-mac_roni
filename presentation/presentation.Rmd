...
slopes <- function(response) {
  return(author_more_than_once %>% 
  select(airline_name, author, y = response,X = date)%>%
  group_by(author,airline_name) %>% # group by variable state
  filter(!is.na(X),!is.na(y))%>%
  do(data.frame(model = tidy(lm(y~X, data=.)))) %>% # for each group run a linear fit and save the output as a date table
  ungroup)
}
```
...
```{r}
author_more_than_once%>%
  filter(!is.na(date),!is.na(.[12]))
```



```{r}
responses = c("seat_comfort_rating",
              "cabin_staff_rating",
...
df2 <- author_more_than_once %>% 
  select(airline_name, author, y = overall_rating,X = date)%>%
  group_by(author,airline_name) %>% # group by variable state
  filter(!is.na(X),!is.na(y))%>%
  do(data.frame(model = tidy(lm(y~X, data=.)))) %>% # for each group run a linear fit and save the output as a date table
...
```{r}
df2 <- joint_improvements%>%
  group_by(airline_name)%>%
  mutate(mean_improvement = mean(overall_rating_improvement))
