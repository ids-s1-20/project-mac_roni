...
After looking into it, that is probably the case! Out of all travelers ( for whom we do have observations),
least number of them chose to fly in the first class cabin. 

```{r-no-of-passengers, echo = FALSE, message = FALSE}
airline %>%
  filter(!is.na(overall_rating), !is.na(cabin_flown) ) %>%
  group_by(cabin_flown) %>%
  count(cabin_flown) %>%
...
```{r-text-analysis-for-filter, echo = FALSE, message = FALSE }
...
The results reveal phrases like "wifi connectivity", "flight entertainment"  don't appear commonly, so I will choose to not look  into the amenity variable associated with those words as it is not ***highly*** spoken about by the first class passengers.

So, after narrowing the search using text analysis, seat, cabin staff and food were used to explore the ratings.

---

```{r-filter-post-text-analysis}
...
```{r-variability, message= FALSE, echo= FALSE }
...
#Observing Overall Correlation

- correlation calculated
- found a positive correlation

```{r calculating the correlation}
abs_data %>%
select(c(3:5)) %>%
  cor()
```
...
To do this, the authors that rated all three were selected and a table was compiled of the ratings so that duplicates were filtered out. Then the difference in the ratings were compared. The graphs show the percentage difference between the authors ratings of lounge and airline, and seat and airline. We can see that in general the percentage differences are quite low despite there being a few exceptions.

To try and see more clearly what the correlations were like we found the correlation coefficient was found and from this we can determine that the seat rating had a slightly stronger correlation to the airline rating as the correlation coefficient is slightly higher than lounge and airline.

Both values are above 0 and closer to 1 than -1 suggesting that both that lounge and seat ratings have a positive correlation to the airline rating which proves our hypothesis to be correct. This then suggests the seat and lounge will have an affect on the authors rating of the airline and flight even though there was only a proportion of authors that rated all three.
